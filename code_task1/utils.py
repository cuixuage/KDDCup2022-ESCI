...
def util_loss_fct(logits, labels, sample_weights, num_labels, loss_name=None):
    ...
    if loss_name == 'mse_loss':
        ...
        # gains = torch.FloatTensor([0.01, 1.0, 0.0, 0.1]).to(torch_device)
        # scores = torch.sum(probs * gains, axis=-1)
        # 方式: 回归
        scores = F.sigmoid(logits)

        loss_fct = MSELoss(reduction='none')    # 2022.05.04 添加sample-weight
        mse_loss = loss_fct(scores.view(-1), labels.view(-1))
        mse_loss = torch.sum(mse_loss * sample_weights) / torch.sum(sample_weights)
        return mse_loss
