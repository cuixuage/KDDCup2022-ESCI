...
os.environ['TRANSFORMERS_CACHE'] = '/home/cuixuange/kddcup_2022/v0.2_train/.cache/huggingface/'
os.environ['HF_DATASETS_CACHE'] = '/home/cuixuange/kddcup_2022/v0.2_train/.cache/huggingface/datasets/'
os.environ['HF_MODULES_CACHE'] = '/home/cuixuange/kddcup_2022/v0.2_train/.cache/huggingface/models/'
os.environ['HF_METRICS_CACHE'] = '/home/cuixuange/kddcup_2022/v0.2_train/.cache/huggingface/metrics/'
...
def main():
    ...
    def preprocess_function_lower(examples):
        sample_weights = [0.5] * len(examples['esci_label'])
        ...
        for idx, _ in enumerate(examples["query"]):
            ...
            title = str(title).translate(str.maketrans('\n', ' ', string.punctuation)).lower().strip() if title and len(title) > 0 and not title.isspace() else ''

            brand = examples["product_brand"][idx]
            brand = str(brand).translate(str.maketrans('\n', ' ', string.punctuation)).lower().strip() if brand and len(brand) > 0 and not brand.isspace() else ''

            color = examples["product_color_name"][idx]
            color = str(color).translate(str.maketrans('\n', ' ', string.punctuation)).lower().strip() if color and len(color) > 0 and not color.isspace() else ''

            bullet_point = examples['product_bullet_point'][idx]
            bullet_point = str(bullet_point).translate(str.maketrans('\n', ' ', string.punctuation)).lower().strip() if bullet_point and len(bullet_point) > 0 and not bullet_point.isspace() else ''

            desc = examples['product_description'][idx]
            if desc and len(desc) > 0 and not desc.isspace():
                desc = re.sub(cleaner, '', desc)
                desc = str(desc).translate(str.maketrans('\n', ' ', string.punctuation)).lower().strip()
            else:
                desc = '' 
        ...
        examples['query'] = [
            line.lower() if line and len(line) > 0 and not line.isspace() else '' for line in examples["query"] 
        ]
    ...
    def preprocess_function(examples):
        ...
        for idx, _ in enumerate(examples["query"]):
            ...
            item = title + ' </s></s> ' + brand + ' </s></s> ' + color + ' </s></s> ' + bullet_point + ' </s></s> ' + desc
            # item = title + ' </s></s> ' + bullet_point + ' </s></s> ' + desc + ' </s></s> ' + brand + ' </s></s> ' + color     # 2022.07.08 reverse-title 测试效果
    ...
    metric_a = load_metric("/home/cuixuange/kddcup_2022/v0.2_train/datasets_metric/accuracy/accuracy.py")
    metric_p = load_metric("/home/cuixuange/kddcup_2022/v0.2_train/datasets_metric/precision/precision.py")
    metric_r = load_metric("/home/cuixuange/kddcup_2022/v0.2_train/datasets_metric/recall/recall.py")
    metric_f = load_metric("/home/cuixuange/kddcup_2022/v0.2_train/datasets_metric/f1/f1.py")
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if step % (518 * args.gradient_accumulation_steps) == 0:   # 0.1 epoch
                model.eval()
                ema.apply_shadow()
    ...
    #     output_test_file = os.path.join(args.output_dir, "test_results.txt.0.7538.singual_language_us")
