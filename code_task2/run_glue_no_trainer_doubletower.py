...
from utils import EMA,get_vocab_from_file, FGM, util_loss_fct, EMA_Without_EmbBag, cos_sim
...
os.environ['TRANSFORMERS_CACHE'] = '/home/kddcup_2022/v0.2_train_tower/.cache/huggingface/'
os.environ['HF_DATASETS_CACHE'] = '/home/kddcup_2022/v0.2_train_tower/.cache/huggingface/datasets/'
os.environ['HF_MODULES_CACHE'] = '/home/kddcup_2022/v0.2_train_tower/.cache/huggingface/models/'
os.environ['HF_METRICS_CACHE'] = '/home/kddcup_2022/v0.2_train_tower/.cache/huggingface/metrics/'
...
def main():
    ...
    emb_bag_vocab = get_vocab_from_file('/home/kddcup_2022/data_process/extra_ngram/emb_bag_vocab.txt')
    if (
    ...
    print("2022-04-10, label_to_id=", label_to_id, args.push_to_hub, padding)   # 2022.04.20 padding = max_length

    def preprocess_function(examples):
        sample_weights = [0.15] * len(examples['esci_label'])

        cleaner = re.compile('<.*?>')    # 删除网页标签, <>内容会被删除
        examples['product_title_bc'] = []
        for idx, _ in enumerate(examples["query"]):
            ...
            item = title + ' </s></s> ' + brand + ' </s></s> ' + color + ' </s></s> ' + bullet_point
            examples['product_title_bc'].append(item)

        examples['query'] = [
            line if line and len(line) > 0 and not line.isspace() else '' for line in examples["query"] 
        ]
        examples['product_description'] = [
            re.sub(cleaner, '', line) if line and len(line) > 0 and not line.isspace() else '' for line in examples["product_description"] 
        ]

        result = dict()
        QBatchEncoding = tokenizer(examples["query"], padding='longest', max_length=args.max_length, truncation=True)  # 2022.05.03 加入翻译后的数据,有些Query长度翻译的很长,不能用only-second阶段方式
        TBC_BatchEncoding = tokenizer(examples["product_title_bc"], padding='longest', max_length=args.max_length, truncation=True)
        TD_BatchEncoding = tokenizer(examples["product_description"], padding='longest', max_length=args.max_length, truncation=True) 
        # print('QBatchEncoding=', type(QBatchEncoding), QBatchEncoding.keys())  #  <class 'transformers.tokenization_utils_base.BatchEncoding'>
        result['query_input_ids'] = QBatchEncoding['input_ids']
        result['query_attention_mask'] = QBatchEncoding['attention_mask']
        result['title_bc_input_ids'] = TBC_BatchEncoding['input_ids']
        result['title_bc_attention_mask'] = TBC_BatchEncoding['attention_mask']
        result['desc_input_ids'] = TD_BatchEncoding['input_ids']
        result['desc_attention_mask'] = TD_BatchEncoding['attention_mask']
    ...
    metric_a = load_metric("/home/kddcup_2022/v0.2_train_tower/datasets_metric/accuracy/accuracy.py")
    metric_p = load_metric("/home/kddcup_2022/v0.2_train_tower/datasets_metric/precision/precision.py")
    metric_r = load_metric("/home/kddcup_2022/v0.2_train_tower/datasets_metric/recall/recall.py")
    metric_f = load_metric("/home/kddcup_2022/v0.2_train_tower/datasets_metric/f1/f1.py")
    ...
    #         # print(batch.keys(), batch['query_input_ids'].shape, batch['query_attention_mask'].shape)
    #         # break
    #         outputs = model(**batch)
    ...
    if args.output_dir is not None:  # 2021.11.10   Predict阶段
        output_test_file = os.path.join(args.output_dir, "test_results.txt.flod_5_tower.0.7114.task2")
