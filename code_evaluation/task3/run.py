...
class Task3Predictor(BasePredictor):
    def __init__(self):
        ...
        self.model = None
        self.tokenizer = None
        self.label_to_id = {"complement": 0, "exact": 0, "irrelevant": 0,"substitute": 1}
    ...
    def prediction_setup(self):
        ...
        """
        num_labels = 2
        model_name_or_path = './models/task3_bs256_0.8218'                 # public=0.8668
        model_name_or_path = './models/task3_bs256_0.8224'                 # public=0.8669
        model_name_or_path = './models/task3_bs256_flod20_0.85299'         # public=0.8677   0.8677755158025441
        model_name_or_path = './models/task3_bs256_flod20_after_cf_0.8429' # public=0.8676
        model_name_or_path = './models/task3_2_classfication_embbag_frompretrained_0.8533' # public=0.8671
        model_name_or_path = './models/task3_2_classfication_embbag_freeze_0.85291' # public=0.8676
        ...
        self.model.config.id2label = {0: 'no_substitute', 1:'substitute'}
    ...
    def predict(self,
        ...
        predict_df['substitute_label'] =  predict_df['prediction'].map(lambda x: change_id_2_class(x))
        submission_df = pd.concat([test_df, predict_df], axis=1)
        submission_df.to_csv(predictions_output_path, columns = ['example_id', 'substitute_label'], encoding='utf8', index=False)
...
if __name__ == "__main__":
    ...
    # test_set_path = "./data/task_3_product_substitute_identification/test_public-v0.3-head1w.csv.zip"
    # product_catalogue_path = "./data/task_3_product_substitute_identification/product_catalogue-v0.3.csv.zip"
    product_catalogue_path = "./data/task_3_product_substitute_identification/task2_task3_product_catalogue-v0.3.csv.zip"

    # Generate a Random File to store predictions
    with tempfile.NamedTemporaryFile(suffix='.csv') as output_file:
        output_file_path = output_file.name
        # output_file_path = "submission_task3_tmp.txt"
