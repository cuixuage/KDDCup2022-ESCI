...
class RobertaWithSampleWeight(RobertaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.emb_bag_dim = 64
        self.char_emb_bag = nn.EmbeddingBag(self.emb_bag_size, self.emb_bag_dim, mode='sum', padding_idx=0)   # pytorch=1.8.2, 不支持padding_idx参数
        self.country_emb = nn.EmbeddingBag(num_embeddings=4, embedding_dim=8, mode='sum', padding_idx=0)
        # self.char_emb_bag = nn.EmbeddingBag(self.emb_bag_size, self.emb_bag_dim, mode='sum')   # pytorch=1.8.2, 不支持padding_idx参数
        # self.country_emb = nn.EmbeddingBag(num_embeddings=4, embedding_dim=8, mode='sum')
    ...
    def forward(
        ...
        # logits, mixed_logits, mixed_labels = self.classifier(sequence_output, labels, brand_idx, color_idx, self.brand_embs, self.color_embs, extend_emb)
...
class BrandColorClassificationHead(nn.Module):
    def __init__(self, config, bc_dim):
        ...
        # self.dense = nn.Linear(config.hidden_size + 64*3 + 8, config.hidden_size)
