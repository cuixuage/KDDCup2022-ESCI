...
def main():
    ...
    if args.dataset_name is not None:
        ...
        # raw_datasets_bc = load_dataset(extension, data_files=data_files_bc)
    ...
    # remove_column_names_bc = raw_datasets_bc['train'].column_names
    ...
    if args.line_by_line:
        ...
        def tokenize_function_fq(examples):
            ...
            for idx, _ in enumerate(examples['faked_query']):
            ...
            examples['faked_query'] = [
                line if line and len(line) > 0 and not line.isspace() else '' for line in examples["faked_query"] 
            ]

            texts = (
                (examples["faked_query"], examples["product_text"])
            )
            result = tokenizer(*texts, padding=padding, max_length=args.mlm_max_seq_length, truncation=True)
        ...
        with accelerator.main_process_first():
            ...
            print("2022-04-21 header= ", tokenized_datasets_train, tokenized_datasets_vaild)
    ...
    # train_bc_dataset = train_bc_dataset["train"]
    eval_dataset = tokenized_datasets_vaild["validation"]

    # Conditional for small test subsets
    if len(train_fq_dataset) > 10:
        for index in random.sample(range(1000), 10):
            ...
            # logger.info(f"Sample {index} of the train_bc_dataset: {train_bc_dataset[index]}.")
    ...
    # train_bc_dataloader = DataLoader(train_bc_dataset, shuffle=True, collate_fn=default_data_collator, batch_size=args.per_device_train_batch_size)
    ...
    # data_loaders = {"FakedQuery":train_fq_dataloader, "BrandColor":train_bc_dataloader, "MLM":train_dataloader}
    # data_iterators = {"FakedQuery":iter(train_fq_dataloader), "BrandColor":iter(train_bc_dataloader), "MLM":iter(train_dataloader)}
    # task_weight = {"FakedQuery":5.0, "BrandColor":0.15, "MLM":1.0}          # 中间变量Loss, 避免三者Loss值差距过大
